...
/**
 * @param calculatedTimerMetrics
 * @param flushInterval, flush interval time span
 * @param ts, time_stamp at which this function get called
 */
const process_metrics = function (metrics, calculatedTimerMetrics, flushInterval, ts, flushCallback) {
    ...
    for (key in timers) {
      ...
      if (timers[key].length > 0) {
		...
		// 这数据结构好啊, 计算百分位直接拿来用就可以了. 取对应idx
        for (let i = 1; i < count; i++) {
			// like fibonacci number, the current := count[idx] + [pri to current]
        ...
        let key2;
		// calc (count, mean, upper, lower, sum, sum_squares) for each percentile
        for (key2 in pctThreshold) {
          ...
          if (count > 1) {
            ...
            if (pct > 0) {
			  ...
			  // looking percentile backwards, which i don't think is necessary at all
		...
		// total timer's min sumSquares, mean 
		// start's to calc stats, min, upper, lower, mean, median
	...
	// the time spended for calc itself
	...
	// assign statsd_metrics
    metrics.statsd_metrics = statsd_metrics;

    flushCallback(metrics);
  };

// only include those inside of `calculatedTimerMetrics`
